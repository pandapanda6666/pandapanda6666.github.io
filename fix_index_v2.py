...
def patch_html(filepath, is_rabboni=False):
    with open(filepath, 'r', encoding='utf-8') as f:
        html = f.read()
    
    # 1. Add green CSS overrides
    green_css = '''
    <style>
        ...
        /* Make sure loading screen is also green in rabboni */
        #splash { background-color: #00c176 !important; }
    </style>
'''
    if 'menu-bar_main-menu_3wqJe' not in html:
        html = html.replace('</head>', green_css + '</head>')
    
    # 2. Add SSO and Settings Menu observer
    sso_logic = '''
    <script>
...
'''
    if 'panda_nickname' not in html:
        html = html.replace('</body>', sso_logic + '</body>')

    # Fix onerror in existing html if any
    html = html.replace(
...
patch_html(os.path.join(base_dir, 'scratch', 'projects', 'editor', 'index.html'))
patch_html(os.path.join(base_dir, 'scratch', 'projects', 'editor', 'rabboni', 'index.html'), is_rabboni=True)
