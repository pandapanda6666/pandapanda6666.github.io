    ...
    def do_POST(self):
        ...
        if path == "/api/export_video":
            ...
            def run_ffmpeg():
                ...
                cmd = ["ffmpeg.exe", "-y", "-i", video_path, "-vf", "scale=-2:" + str(resolution) + ",ass='" + safe_ass.replace(':', '\\\\:') + "'", "-c:v", "libx264", "-preset", "fast", "-crf", str(crf), "-c:a", "copy", out_path]
                
                dur_cmd = ["ffmpeg.exe", "-i", video_path]
