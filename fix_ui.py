...
/* 修正原生 Navbar 的 hover 與 active 藍色背景，改為透明黑底 */
div[class*="menu-bar_hoverable_"]:hover, 
div[class*="menu-bar_active_"] {
...
function logout() {
    ...
    localStorage.removeItem('panda_username');
    localStorage.removeItem('panda_avatar');
...
const observer = new MutationObserver((mutations) => {
    ...
    if (menuBar && !document.getElementById('custom-sso-nav')) {
        ...
        // 使用 panda_session_token 或 sso_auth 來判斷是否登入
        if (localStorage.getItem('sso_auth') === 'true' || localStorage.getItem('panda_session_token')) {
            // 讀取 panda_username，若無則讀取 sso_user，最後預設 User
            const username = localStorage.getItem('panda_username') || localStorage.getItem('sso_user') || 'User';
            // 使用原版的 P幣 或 用戶自訂頭像
            const defaultAvatar = 'https://pandapanda6666.github.io/static/assets/pandacoin.png';
            const avatar = localStorage.getItem('panda_avatar') || defaultAvatar;
            ...
            authContainer.innerHTML = `
                <div class="avatar-container menu-bar_menu-bar-item_264qQ menu-bar_hoverable_2sbwj" style="display: flex; align-items: center; cursor: pointer; height: 100%; position: relative; padding: 0 15px;">
                    ...
                    <span class="username" style="color: white; font-size: 0.85rem; font-weight: bold;">${username}</span>
                    <ul class="dropdown-content menu_menu_3hsw_ menu-bar_menu_3tWvA" style="display: none; position: absolute; top: 100%; right: 0; min-width: 160px; z-index:10001; margin:0; padding:0; list-style:none;">
                        <li class="menu_menu-item_3EwYA menu-bar_menu-item_2kP1R" onclick="window.location.href='https://pandapanda6666.github.io/login-hub/?action=editProfile'"><span>個人資訊</span></li>
                        <li class="menu_menu-item_3EwYA menu-bar_menu-item_2kP1R" onclick="window.location.href='/scratch/mystuff/'"><span>我的東西</span></li>
                        <li class="menu_menu-item_3EwYA menu-bar_menu-item_2kP1R" onclick="window.location.href='/scratch/settings/'"><span>帳戶設定</span></li>
    ...
    if (langMenu && !document.getElementById('custom-settings-menu')) {
        ...
        // 使用原生 Scratch 的 hover class (與檔案/編輯同級)
        ...
        // 渲染原生風格的選單 (包含 ul.menu_menu_3hsw_)
