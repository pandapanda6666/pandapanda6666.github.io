...
new_inject = '''<!-- PANDASCRATCH INJECT START -->
<style>
/* 隱藏原生多餘元素與登入區塊 */
...
.panda-custom-menu:hover .panda-dropdown {
    display: block;
}
.panda-dropdown li {
    padding: 10px 15px;
...
document.addEventListener('DOMContentLoaded', () => {
    const observer = new MutationObserver((mutations) => {
        ...
        // 點擊 Scratch Logo 回首頁
        const logoImg = document.querySelector('img[class*="menu-bar_scratch-logo_"]');
        if (logoImg && !logoImg.dataset.injected) {
            ...
            if (localStorage.getItem('sso_auth') === 'true' || localStorage.getItem('panda_session_token')) {
                ...
                authWrapper.innerHTML = \
                    ...
                    <div class="panda-custom-menu">
                        <img src="\" style="width:32px; height:32px; border-radius:50%; margin-right:8px; border:1px solid rgba(255,255,255,0.4); background:#fff; object-fit: cover;">
                        <span style="display: flex; align-items: center;">
                            \
                            <span style="margin-left: 10px; color: gold; display: flex; align-items: center;">
                                <img src="https://pandapanda6666.github.io/shop/coin.svg" style="width:16px;height:16px;margin-right:2px;" onerror="this.onerror=null; this.src='https://pandapanda6666.github.io/login-hub/pandacoin.png'" />\
                            </span>
                        </span>
...
if start_idx != -1 and content.find(end_marker) != -1:
    new_content = content[:start_idx] + new_inject + content[end_idx:]
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Successfully injected CSS and fixed mystuff.png.")
else:
    print("Injection markers not found.")
