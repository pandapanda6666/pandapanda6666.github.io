...
# 1. Add CSS for Nested Submenus and Account Active State
css_injection = '''
  /* Custom Settings Menu CSS */
  .panda-settings-btn {
      position: relative;
      cursor: pointer;
      display: flex;
  ...
  .panda-settings-btn:hover, .panda-settings-btn.active {
      background-color: var(--panda-dark-green) !important;
  }
  .panda-dropdown {
      display: none;
      position: absolute;
  ...
  .panda-settings-btn.active > .panda-dropdown, .panda-sso-nav:hover > .panda-dropdown, .panda-sso-nav.active > .panda-dropdown {
      display: block;
  }
  .panda-dropdown li {
      position: relative;
  ...
  /* Open submenu on hover of the parent li */
  .panda-dropdown li:hover > .panda-submenu {
      display: block;
  }
  /* Checkmark icons */
  .panda-check { visibility: hidden; margin-left: 10px; }
  .panda-check.active { visibility: visible; }
  
  .panda-hidden-lang { display: none !important; }
'''

# Find where to inject CSS
content = content.replace('.panda-settings-btn {', css_injection + '\n  /* Old CSS replaced */\n  .panda-settings-btn_old {')

# 2. Inject JS for Settings Menu and Account Menu
js_injection = '''
                  ...
                  <ul class="panda-dropdown left">
                      ...
                      <li onmousedown="event.stopPropagation()">
                          ...
                          <ul class="panda-submenu">
                              <li id="btn-theme-default" onclick="event.stopPropagation();">Default <span class="panda-check ">✔</span></li>
                              <li id="btn-theme-bear" onclick="event.stopPropagation();">Cat Blocks (熊積木) <span class="panda-check ">✔</span></li>
                          </ul>
                      </li>
                      <li onmousedown="event.stopPropagation()">
                          ...
                          <svg class="panda-icon" viewBox="0 0 256 512" width="10" height="10" fill="currentColor"><path d="M246.6 278.6c12.5-12.5 12.5-32.8 0-45.3l-128-128c-9.2-9.2-22.9-11.9-34.9-6.9s-19.8 16.6-19.8 29.6l0 256c0 12.9 7.8 24.6 19.8 29.6s25.7 2.2 34.9-6.9l128-128z"/></svg>
                          <ul class="panda-submenu">
                              <li id="btn-contrast-original" onclick="event.stopPropagation();">原始 <span class="panda-check ">✔</span></li>
                              <li id="btn-contrast-high" onclick="event.stopPropagation();">高對比 <span class="panda-check ">✔</span></li>
                          </ul>
                      </li>
                  </ul>;
...
content = content.replace("          if (menuBar && !document.getElementById('custom-sso-nav-wrapper')) {", js_injection + "\n          if (menuBar && !document.getElementById('custom-sso-nav-wrapper')) {")

# Version bump
content = content.replace('v=84', 'v=85')
