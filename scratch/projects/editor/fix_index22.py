...
content = re.sub(r"([ \t]*)if \(menuBar && !document\.getElementById\('custom-sso-nav-wrapper'\)\) \{", 
                 lambda m: m.group(1) + js_injection.strip() + "\n\n" + m.group(1) + "if (menuBar && !document.getElementById('custom-sso-nav-wrapper')) {", 
                 content, 
                 count=1)

# Bump version to 87
content = content.replace('v=86', 'v=87')
